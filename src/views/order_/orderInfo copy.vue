    ...
    <div class="item">
      ...
      <div class="table">
        ...
        <div class="tbody" style="padding: 15px 0 15px 20px">
          <span style="color: #1b2ba6">商家备注：{{ orderInfo.info.bak==""?"无": orderInfo.info.bak}}</span>
        </div>
      </div>
      <el-row :gutter="20" class="goodsindex-list" style="margin: 0 0 0 0">
        <el-col :span="24" style="padding: 0; margin-top: 20px">
          <el-table :data="orderInfo.info.product_list" border :span-method="cellMerge2">
            ...
            <el-table-column label="售后详情" width="150">
              <template slot-scope="scope">
                <div v-if="scope.row.return_status != 0">
                  <div>退款中: {{scope.row.refun_of}}件</div>
                  <div>退款完成: {{scope.row.refund_complete}}件</div>
                  <el-button
                    type="success"
                    style="margin-bottom: 10px"
                    @click="lookRefund(orderInfo.info.order_no)"
                    >查看</el-button
                  >
                </div>
          ...
          <el-table :data="order_s_list1" border :span-method="cellMerge">
            ...
            <el-table-column label="买家信息">
              <template slot-scope="scope">
                ...
                <div v-else>
                  <div>昵称: {{orderInfo.info.user_info.nickname}}</div>
                  <div>账号: {{orderInfo.info.user_info.phone}}</div>
                  <div>UID: {{orderInfo.info.user_info.uid}}</div>
                  <div>区域代理: 暂无</div>
                  <div>销售代理: 
    ...
    <div class="item item1">
      <div class="head clearfix">
        ...
        <el-button
          class="rt"
          type="info"
          style="margin-bottom: 10px;cursor:no-drop;"
          v-else
        >
          发货
        </el-button>
      </div>
      <div v-for="(item,index) in orderInfo.info.package" :key="index">
        <p style="margin: 20px 0 0" v-if="index == 0">默认分包</p>
        <p style="margin: 20px 0 0" v-else>新增分包{{index}}</p>
        <el-row :gutter="20" class="goodsindex-list" style="margin: 0 0 0 0">
          <el-col :span="24" style="padding: 0; margin-top: 20px">
            <el-table :data="item.product_list" border  :span-method="cellMerge3">
    ...
    info(){
      ...
      .then((res) => {
        ...
        if (res.data.err_code == 0) {
          ...
          if(res.data.err_msg.status){
            
          }else{
            res.data.err_msg.status = this.status
          }
          ...
          console.log(res.data.err_msg.status)
